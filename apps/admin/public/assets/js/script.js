...
$(document).ready(function(){
	...
	if ($('.datatable').length > 0) {
		$('.datatable').each(function () {
			// Destroy existing DataTable instance if it already exists
			...
			$(this).DataTable({
				...
				initComplete: function (settings, json) {
					// Move elements to custom containers (if they exist)
...
$(document).ready(function(){
	...
	if ($('.bookingrange').length > 0) {
		...
		const end = moment();
		const updateBookingRange = function (start, end) {
			$('.bookingrange span').html(start.format('M/D/YYYY') + ' - ' + end.format('M/D/YYYY'));
		};
		$('.bookingrange').daterangepicker({
			startDate: start,
	...
	if($('.flat-datepickr').length > 0 ){		
		flatpickr(".flat-datepickr", {
			dateFormat: "d-m-Y",
		});
	}
	if ($('.flat-timepickr').length > 0) {
		flatpickr(".flat-timepickr", {
			enableTime: true,
			noCalendar: true,
			dateFormat: "H:i", // Time format in 24-hour notation
			time_24hr: true // Ensures the picker uses a 24-hour clock
		});
	}
	if($('.flat-monthpickr').length > 0 ){		
		flatpickr(".flat-monthpickr", {
		...
		'<div class="row align-items-center g-2">'+
			...
			'<div class="col-lg-2">'+
				'<div class="d-flex align-items-center gap-2">'+
					'<button class="btn bg-dark border border-dark text-white text-center hover:bg-black hover:text-white w-100">Charge</button>'+
	...
	$(document).ready(function() {
		...
		$('[data-provider="flatpickr"]').each(function() {
			const $element = $(this);
			...
			const enableTime = $element.attr('data-enable-time');
			if (enableTime) {
			...
			const altFormat = $element.attr('data-alt-format');
			if (altFormat) { 
			...
			const defaultDate = $element.attr('data-deafult-date'); 
			if (defaultDate) { 
				options.defaultDate = defaultDate;
			}
	
			const disableDate = $element.attr('data-disable-date'); 
			if (disableDate) { 
				
				options.disable = [disableDate]; 
			}
	
			const multipleDate = $element.attr('data-multiple-date');
			if (multipleDate) {
				options.mode = "multiple";
			}
	
			const rangeDate = $element.attr('data-range-date');
			if (rangeDate) {
				options.mode = "range";
			}
	
			const inlineDate = $element.attr('data-inline-date');
			if (inlineDate) {
				options.inline = true;
			}
	
			const weekNumber = $element.attr('data-week-number');
			if (weekNumber) {
				options.weekNumbers = true;
			}
			
			flatpickr($element[0], options);
		});
		
		$('[data-provider="timepickr"]').each(function() {
			const $element = $(this);
			...
			const timeHrs = $element.attr('data-time-hrs');
			const minTime = $element.attr('data-min-time');
			const defaultTime = $element.attr('data-default-time');
			const timeInline = $element.attr('data-time-inline');
	
			if (timeHrs || minTime || defaultTime || timeInline) {
			...
			if (timeInline) {
				...
				options.defaultDate = timeInline; 
			}
	...
	// Sidebar
	var Sidemenu = function() {
		this.$menuItem = $('.sidebar-menuOne a');
	};
	
	function init() {
		var $this = Sidemenu;
		$('.sidebar-menuOne a').on('click', function(e) {
			if($(this).parent().hasClass('submenu')) {
				e.preventDefault();
			}
			if(!$(this).hasClass('subdrop')) {
		...
		$('.sidebar-menuOne ul li.submenuOne a.active').parents('li:last').children('a:first').addClass('active').trigger('click');
	}
	...
	$(document).ready(function() {
		...
		$tabs.on("click", function() {
		  ...
		  $tabs.removeClass("active text-teal-600 border-b-2 border-teal-600");
		  $contents.addClass("hidden");
	  
		  $(this).addClass("active text-teal-600 border-b-2 border-teal-600");
	  
		  $(`.tab_content[data-tab='${target}']`).removeClass("hidden");
		});
	  });
